...
namespace gk2a
{
    namespace uhrit
    {
        ...
        GK2AUHRITCADUextractor::GK2AUHRITCADUextractor(std::string input_file, std::string output_file_hint, nlohmann::json parameters) : ProcessingModule(input_file, output_file_hint, parameters)
        {
            bb_buffer = new uint8_t[BBFRAME_SIZE];
            cadu_buffer = new uint8_t[CADU_SIZE];
        }

        std::vector<ModuleDataType> GK2AUHRITCADUextractor::getInputTypes()
        {
            return {DATA_FILE, DATA_STREAM};
        }

        std::vector<ModuleDataType> GK2AUHRITCADUextractor::getOutputTypes()
        {
            return {DATA_FILE, DATA_STREAM};
        }
        ...
        {
            ImGui::Begin("GK-2A UHRIT CADU Extractor", NULL, window ? 0 : NOWINDOW_FLAGS);
            ImGui::BeginGroup();
            {
        ...
        std::string GK2AUHRITCADUextractor::getID()
        {
            return "gk2a_uhrit_cadu_extractor";
        }

        std::vector<std::string> GK2AUHRITCADUextractor::getParameters()
        {
            return {};
        }

        std::shared_ptr<ProcessingModule> GK2AUHRITCADUextractor::getInstance(std::string input_file, std::string output_file_hint, nlohmann::json parameters)
        {
